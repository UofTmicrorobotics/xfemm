...
int execLuaFile(
  ...
  const std::string & luaInit,
  bool luaTrace,
  const std::string & luaBaseDir,
  bool luaPedanticMode,
  bool luaDebugGeometry)
{
...
int main(int argc, char ** argv)
{
  ...
  std::string luaInit = location::locateFile(
    location::LocationType::SystemData,
    "xfemm",
    "release/init.lua");
  bool luaTrace = false;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    std::cout << "\n";
    std::cout << "Usage: " << exe <<
      " [-q|--quiet] [--lua-trace-functions] [--lua-pedantic-mode] [--lua-init=<init.lua>] [--lua-base-dir=<dir>] --lua-script=<file.lua>\n";
    ...
    std::cout <<
      " --lua-debug-geometry     Debug lua functions that change the geometry of the model\n";
