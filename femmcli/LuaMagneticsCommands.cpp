...
int femmcli::LuaMagneticsCommands::luaAddBHPoint(lua_State * L)
{
  ...
  std::shared_ptr<FemmState> femmState = std::dynamic_pointer_cast<FemmState>(
    luaInstance->femmState());
...
{
  std::shared_ptr<FemmState> femmState =
    std::dynamic_pointer_cast<FemmState>(LuaInstance::instance(L)->femmState());
...
int femmcli::LuaMagneticsCommands::luaProblemDefinition(lua_State * L)
{
  ...
  if (units == "inches") {magDoc->LengthUnits = LengthInches;} else if (units == "millimeters") {
    magDoc->LengthUnits = LengthMillimeters;
  } else if (units == "centimeters") {
    magDoc->LengthUnits = LengthCentimeters;
  } else if (units == "meters") {magDoc->LengthUnits = LengthMeters;} else if (units == "mills") {
    magDoc->LengthUnits = LengthMils;
  } else if (units == "mils") {magDoc->LengthUnits = LengthMils;} else if (units == "micrometers") {
    magDoc->LengthUnits = LengthMicrometers;
  } else {
  ...
  if (type == "planar") {magDoc->problemType = PLANAR;} else if (type == "axi") {
    magDoc->problemType = AXISYMMETRIC;
  } else {
    ...
    if (abs(x - y) < 1.e-08) {
      for (int k = 0; k < (int)fpproc->arclist.size(); k++) {
        if ((fpproc->arclist[k].n0 == n1) && (fpproc->arclist[k].n1 == n0)) {
          double d2 = fpproc->ShortestDistanceFromArc(
            CComplex(mx, my),
            fpproc->arclist[k]);
          if (d2 < d1) {
            arcno = k;
