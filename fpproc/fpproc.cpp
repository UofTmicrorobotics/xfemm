  ...
  while ((flag == false) && (fgets(s, 1024, fp) != NULL)) {
    ...
    if (_strnicmp(q, "[lengthunits]", 13) == 0) {
      ...
      sscanf(
        ...
        "%s",
        q);
      if (_strnicmp(q, "inches", 6) == 0) {LengthUnits = 0;} else if (_strnicmp(
          q, "millimeters",
          11) == 0)
      {
        LengthUnits = 1;
...
//                }
    ...
    if (_strnicmp(q, "[numblocklabels]", 13) == 0) {
      ...
      for (i = 0; i < k; i++) {
        ...
        if (blk.MaxArea < 0) {blk.MaxArea = 0;} else {
    ...
    if (fgets(s, 1024, fp) != NULL) {
      if (Frequency != 0) {
        if (!bIncremental) {
          ...
          if (sscnt != 4) {
            std::string msg =
              "An error occured while reading mesh nodes section of file, wrong number of inputs ("
              +
  ...
  for (i = 0; i < k; i++) {
    ...
    if (Frequency == 0) {
      sscanf(s, "%i\t%lf", &j, &zr);
      blocklist[i].Case = j;
      if (j == 0) {blocklist[i].dVolts = zr;} else {blocklist[i].J = zr;}
    } else {
      sscanf(s, "%i\t%lf\t%lf", &j, &zr, &zi);
      blocklist[i].Case = j;
      if (j == 0) {blocklist[i].dVolts = zr + I * zi;} else {blocklist[i].J = zr + I * zi;}
    }
  }
  ...
  for (i = 0; i < k; i++) {
    ...
    fgets(s, 1024, fp);
    sscanf(
      s, "%i %lf %lf %lf %lf %lf %lf %lf %i %lf %lf",
  ...
  for (i = 0; i < (int)agelist.size(); i++) {
    ...
    for (k = 0; k < agelist[i].totalArcElements; k++) {
      ...
      if (agelist[i].BdryFormat == 0) {
        ac =
          (2 * a[2] + 2 * a[3] + 2 * a[7] + 2 * a[8] + a[1] * ci + (a[2] - a[3] - a[4]) * ci -
          (a[0] - 3 * a[1] + a[2] + 3 * a[3] - 2 * a[4]) *
          std::pow(
            ci,
            2) + (a[0] - 2 * a[1] + 2 * a[3] - a[4]) *
          std::pow(ci, 3) + (a[6] + a[7] - a[8] - a[9]) * co -
          (a[5] - 3 * a[6] + a[7] + 3 * a[8] - 2 * a[9]) *
          std::pow(co, 2) + (a[5] - 2 * a[6] + 2 * a[8] - a[9]) * std::pow(co, 3)) / 8.;
        agelist[i].aco += ac / ((double) agelist[i].totalArcElements);
      }

      // flux density for this element
      agelist[i].br[k] =
        (-(ci * a[1]) - 2 * a[2] + 2 * a[3] + ci * (a[2] + a[3] - a[4]) - ci * ci * ci *
        (a[0] - 4 * a[1] + 6 * a[2] - 4 * a[3] + a[4]) + ci * ci *
        (a[0] - 5 * a[1] + 9 * a[2] - 7 * a[3] + 2 * a[4]) - 2 * a[7] +
        2 * a[8] + co * (-a[6] + a[7] + a[8] - a[9]) - co * co * co *
        (a[5] - 4 * a[6] + 6 * a[7] - 4 * a[8] + a[9]) + co * co *
        (a[5] - 5 * a[6] + 9 * a[7] - 7 * a[8] + 2 * a[9])) / (4 * dt * R);
      agelist[i].bt[k] =
        (ci * a[1] + 2 * a[2] + 2 * a[3] - ci * ci *
        (a[0] - 3 * a[1] + a[2] + 3 * a[3] - 2 * a[4]) + ci * (a[2] - a[3] - a[4]) + ci * ci * ci *
        (a[0] - 2 * a[1] + 2 * a[3] - a[4]) - co * a[6] +
        (-2 + co) * (1 + co) * a[7] - 2 * a[8] + co *
        (a[8] + co * (a[5] - 3 * a[6] + 3 * a[8] - 2 * a[9]) + a[9] + co * co *
        (-a[5] + 2 * a[6] - 2 * a[8] + a[9]))) / (4 * dr);
      if (bIncremental) {
        for (kk = 0; kk < 10; kk++) {
          a[kk] = meshnode[nn[kk]].Aprev * ww[kk];
        }

        agelist[i].brPrev[k] = Re(
          (-(ci * a[1]) - 2 * a[2] + 2 * a[3] + ci * (a[2] + a[3] - a[4]) - ci * ci * ci *
          (a[0] - 4 * a[1] + 6 * a[2] - 4 * a[3] + a[4]) + ci * ci *
          (a[0] - 5 * a[1] + 9 * a[2] - 7 * a[3] + 2 * a[4]) - 2 * a[7] +
          2 * a[8] + co * (-a[6] + a[7] + a[8] - a[9]) - co * co * co *
          (a[5] - 4 * a[6] + 6 * a[7] - 4 * a[8] + a[9]) + co * co *
          (a[5] - 5 * a[6] + 9 * a[7] - 7 * a[8] + 2 * a[9])) / (4 * dt * R));
        agelist[i].btPrev[k] = Re(
          (ci * a[1] + 2 * a[2] + 2 * a[3] - ci * ci *
          (a[0] - 3 * a[1] + a[2] + 3 * a[3] - 2 * a[4]) + ci * (a[2] - a[3] - a[4]) + ci * ci *
          ci * (a[0] - 2 * a[1] + 2 * a[3] - a[4]) - co * a[6] +
          (-2 + co) * (1 + co) * a[7] - 2 * a[8] + co *
          (a[8] + co * (a[5] - 3 * a[6] + 3 * a[8] - 2 * a[9]) + a[9] + co * co *
          (-a[5] + 2 * a[6] - 2 * a[8] + a[9]))) / (4 * dr));
      }
    ...
    for (j = 0; j < agelist[i].nn; j++) {
      if (agelist[i].BdryFormat == 0) {agelist[i].nh[j] = m * j;} else {
      ...
      if ((agelist[i].nh[j] == 0) ||
        (((j == (agelist[i].nn - 1)) && (agelist[i].BdryFormat == 0)) &&
        ((agelist[i].totalArcElements % 2) == 0)))
      {
...
//            str = fmatter.str();
      ...
      SNPRINTF(
        magbuff, 4096, "x=%.17g\ny=%.17g\nr=x\nz=y\ntheta=%.17g\nR=%.17g\nreturn %s",
        (X.re), (X.im), (arg(X) * 180 / PI), (abs(X)),
        (blocklist[meshelem[i].lbl].MagDirFctn.c_str() ) );
      str = magbuff;
  ...
  // Find extreme values of B and H;
  {
    ...
    for (i = 0; i < (int)meshelem.size(); i++) {
      ...
      for (j = 0; j < 3; j++) {
        br = sqrt(
          sqr(meshelem[i].b1[j].re) +
          sqr(meshelem[i].b2[j].re));
        bi = sqrt(
          sqr(meshelem[i].b1[j].im) +
...
//    }
  ...
  // permanent magnets with a nonlinear demagnetization curve
    #ifdef DEBUG_FPPROC
  printf(
    "Get some information needed to compute energy stored in permanent magnets with a nonlinear demagnetization curve.\n");
  fflush(stdout);
    #endif
  if (Frequency == 0) {
    for (k = 0; k < (int)blockproplist.size(); k++) {
      if ((blockproplist[k].H_c > 0) && (blockproplist[k].BHpoints > 0)) {
        blockproplist[k].Nrg =
          blockproplist[k].GetCoEnergy(blockproplist[k].GetB(blockproplist[k].H_c));
      }
    }
  }
      ...
      if (blocklist[lbl].Case == 0) {
        if (problemType == PLANAR) {
          ...
          for (tn = 0; tn < 3; tn++) {
            ...
            if (R[tn] < 1.e-6) {R[tn] = ravg;} else {R[tn] *= LengthConv[LengthUnits];}
          }
    ...
    if (blockproplist[meshelem[k].blk].H_c != 0) {
      ...
      if (blockproplist[bk].BHpoints == 0) {
        ...
        u.E = u.E + blockproplist[bk].Nrg -
          blockproplist[bk].H_c *
          Re((u.B1.re + I * u.B2.re) / exp(I * PI * meshelem[k].magdir / 180.));
      }
...
//                Z[i]=meshnode[n[i]].y;
    ...
    if (blockproplist[meshelem[k].blk].LamType > 2) {
      ...
      u.E += Re(J * conj(J)) * (Im(1. / blocklist[meshelem[k].lbl].o) / (2.e6 * PI * Frequency)) /
        4.;
    }
...
void FPProc::GetPointB(
  const double x, const double y, CComplex & B1, CComplex & B2,
...
void FPProc::GetNodalB(CComplex * b1, CComplex * b2, femmpostproc::CPostProcMElement & elm)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0, m = 0; j < NumList[k]; j++) {
      if (elm.lbl == meshelem[ConList[k][j]].lbl) {m++;} else {
        if (Frequency == 0) {
          if ((blockproplist[elm.blk].mu_x ==
            ...
            (elm.magdir == meshelem[ConList[k][j]].magdir))
          {
            m++;
      ...
      if (flag == false) {
        ...
        for (j = 0; j < NumList[k]; j++) {
          if (elm.lbl == meshelem[ConList[k][j]].lbl) {
            m = ConList[k][j];
            bt.re = sqrt(
              meshelem[m].B1.re * meshelem[m].B1.re +
              meshelem[m].B2.re * meshelem[m].B2.re);
            bt.im = sqrt(
              meshelem[m].B1.im * meshelem[m].B1.im +
...
//            Z[i]=meshnode[n[i]].y;
    ...
    // construct values for mid-side nodes;
    if ((R[0] < 1.e-06) && (R[1] < 1.e-06)) {v[1] = (v[0] + v[2]) / 2.;} else {
      v[1] = (R[1] * (3. * v[0] + v[2]) + R[0] * (v[0] + 3. * v[2])) /
        (4. * (R[0] + R[1]));
    }

    if ((R[1] < 1.e-06) && (R[2] < 1.e-06)) {v[3] = (v[2] + v[4]) / 2.;} else {
      v[3] = (R[2] * (3. * v[2] + v[4]) + R[1] * (v[2] + 3. * v[4])) /
        (4. * (R[1] + R[2]));
    }

    if ((R[2] < 1.e-06) && (R[0] < 1.e-06)) {v[5] = (v[4] + v[0]) / 2.;} else {
      v[5] = (R[0] * (3. * v[4] + v[0]) + R[2] * (v[4] + 3. * v[0])) /
        (4. * (R[2] + R[0]));
    }
...
CComplex FPProc::GetJA(int k, CComplex * J, CComplex * A) const
{
  ...
  // first, get A
  for (i = 0; i < 3; i++) {
    if (problemType == PLANAR) {A[i] = (meshnode[meshelem[k].p[i]].A);} else {
      rn = meshnode[meshelem[k].p[i]].x * LengthConv[LengthUnits];
      if (fabs(rn / LengthConv[LengthUnits]) < 1.e-06) {A[i] = 0;} else {
      ...
      if (blocklist[meshelem[i].lbl].IsSelected == true) {
        ...
        switch (inttype) {
          ...
          case 11:       // x (or r) direction Lorentz force, SS part.
            ...
            if (problemType == AXISYMMETRIC) {y = 0;} else {y *= Depth;}
          ...
          case 14:       // y (or z) direction Lorentz force, 2x part.
            if (Frequency != 0) {
              ...
              if (problemType == AXISYMMETRIC) {y = (-y * 2. * PI * R);} else {y *= Depth;}
              z += (a * y) / 2.;
            }
            break;
          ...
          case 2:       // stored energy
            if (problemType == AXISYMMETRIC) {a *= (2. * PI * R);} else {a *= Depth;}
            B1 = meshelem[i].B1;
            B2 = meshelem[i].B2;
            if (Frequency != 0) {
              ...
              if (blockproplist[meshelem[i].blk].H_c != 0) {
                ...
                if (blockproplist[bk].BHpoints == 0) {
                  ...
                  y = y + blockproplist[bk].Nrg -
                    blockproplist[bk].H_c *
                    Re((B1.re + I * B2.re) / exp(I * PI * meshelem[i].magdir / 180.));
                  y *= a;
                }
      ...
      if ((inttype >= 18) || (inttype <= 23)) {
        ...
        switch (inttype) {
          ...
          case 18:       // x (or r) direction Henrotte force, SS part.
            ...
            y = (((B1 * conj(B1)) - (B2 * conj(B2))) * Re(c) + 2. * Re(B1 * conj(B2)) * Im(c)) /
              (2. * muo);
          ...
          case 19:       // y (or z) direction Henrotte force, SS part.
            ...
            y = (((B2 * conj(B2)) - (B1 * conj(B1))) * Im(c) + 2. * Re(B1 * conj(B2)) * Re(c)) /
              (2. * muo);
          ...
          case 20:       // x (or r) direction Henrotte force, 2x part.
            ...
            B2 = meshelem[i].B2;
            c = HenrotteVector(i);
            z += a * ((((B1 * B1) - (B2 * B2)) * Re(c) + 2. * B1 * B2 * Im(c)) / (4. * muo)) * AECF(
              i);
          ...
          case 21:       // y (or z) direction Henrotte force, 2x part.
            ...
            B2 = meshelem[i].B2;
            c = HenrotteVector(i);
            z += a * ((((B2 * B2) - (B1 * B1)) * Im(c) + 2. * B1 * B2 * Re(c)) / (4. * muo)) * AECF(
              i);
...
CComplex FPProc::GetStrandedVoltageDrop(int lbl) const
{
  ...
  for (i = 0, dVolts = 0, atot = 0; i < (int)meshelem.size(); i++) {
    if (meshelem[i].lbl == lbl) {
      ...
      if (problemType == PLANAR) {dVolts += PlnInt(a, V, U) * Depth;} else {
...
void FPProc::GetFillFactor(int lbl)
{
  ...
  if (wiretype == 3) {  // rectangular wire
    ...
    if (o != 0) {
      ...
      bl->o = 1. /
        (muo / (fill * o * ufd) + I * dd * dd * (1. - fill) * muo * W / 4. - I * dd * dd * ueff *
        W / 12.);
  ...
  if (bp->Cduct == 0) {
    ...
    // locally stored energy in the winding due to the disribution of turns.
    bl->o = 1. /
      (((I / 2.) * w * muo * R * R * log(1.5299240194394943 / sqrt(fill))) / fill - (I / 12.) *
      muo * dd * dd);
    bl->mu = 1;
    return;
  }
  ...
  c1 = 0.7756067409818643 + fill *
    (0.6873854335408803 + fill * (0.06841584481674128 - 0.07143732702512284 * fill));
  c2 = 1.5 * fill / c1;
  ...
  c3 = 0.8824642871525136 + fill *
    (-0.008605512994838827 + fill * (0.7223208744682307 - 0.2157183942377177 * fill));
...
CComplex FPProc::GetStrandedLinkage(int lbl) const
{
  ...
  for (i = 0, FluxLinkage = 0, atot = 0; i < (int)meshelem.size(); i++) {
    if (meshelem[i].lbl == lbl) {
      ...
      if (problemType == PLANAR) {FluxLinkage += PlnInt(a, A, U) * Depth;} else {
    ...
    for (i = 0, FluxLinkage = 0; i < (int)meshelem.size(); i++) {
      if (blocklist[meshelem[i].lbl].InCircuit == circnum) {
        ...
        if (Im(blocklist[meshelem[i].lbl].o) != 0) {
          double u;
          if (Frequency == 0) {u = Im(blocklist[meshelem[i].lbl].o);} else {
            u = Im(1.e-6 / blocklist[meshelem[i].lbl].o) / (2. * PI * Frequency);
          }
          for (k = 0; k < 3; k++) {
            A[k] += u * J[k];
          }
        ...
        if (problemType == PLANAR) {FluxLinkage += PlnInt(a, A, J) * Depth;} else {
...
FPProcError FPProc::gapDCTorqueIntegral(const std::string myBdryName, double & tq) const
{
  ...
  for (k = 0; k < agelist[i].nn; k++) {
    tq += Re(
      agelist[i].brc[k] * conj(agelist[i].btc[k]) +
...
FPProcError FPProc::gapDCForceIntegral(
  const std::string myBdryName, CComplex & fx,
  CComplex & fy) const
{
  int i, k;
  ...
  if (round(agelist[i].totalArcLength) == 360) {
    for (k = 1; k < agelist[i].nn; k++) {
      ...
      dfx =
        (((agelist[i].brs[k] + agelist[i].btc[k]) *
        conj(agelist[i].brs[k - 1] - agelist[i].btc[k - 1]) +
        (agelist[i].brs[k - 1] - agelist[i].btc[k - 1]) *
        conj(agelist[i].brs[k] + agelist[i].btc[k]) +
        (agelist[i].brc[k] - agelist[i].bts[k]) *
        conj(agelist[i].brc[k - 1] + agelist[i].bts[k - 1]) +
        (agelist[i].brc[k - 1] + agelist[i].bts[k - 1]) *
        conj(agelist[i].brc[k] - agelist[i].bts[k])));
...
FPProcError FPProc::gap2XForceIntegral(
  const std::string myBdryName, CComplex & fx,
  CComplex & fy) const
{
  int i, k;
  ...
  if ((round(agelist[i].totalArcLength) == 360) && (Frequency != 0)) {
    for (k = 1; k < agelist[i].nn; k++) {
      ...
      dfx = (agelist[i].brs[k - 1] - agelist[i].btc[k - 1]) *
        (agelist[i].brs[k] + agelist[i].btc[k]) +
        (agelist[i].brc[k - 1] + agelist[i].bts[k - 1]) * (agelist[i].brc[k] - agelist[i].bts[k]);

      dfy = (agelist[i].brs[k] + agelist[i].btc[k]) *
        (agelist[i].brc[k - 1] + agelist[i].bts[k - 1]) -
...
FPProcError FPProc::gapIncrementalForceIntegral(
  const std::string myBdryName,
  CComplex & fx,
  CComplex & fy) const
{
  int i, k;
...
FPProcError FPProc::getGapHarmonics(
  const std::string myBdryName,
  const int n,
  CComplex & acc,
  CComplex & acs,
