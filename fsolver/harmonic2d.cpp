    ...
    for (i = 0; i < NumCircProps; i++) {
      ...
      if (circproplist[i].CircType == 0) {   // specified current
        if (CircInt2[i] == 0) {     // circuit composed of zero cond. materials
          ...
          if (CircInt1[i] == 0.) {circproplist[i].J = 0.;} else {
            circproplist[i].J = 0.01 * (
      ...
      for (j = 0; j < 3; j++) {
        if (El->e[j] >= 0) {
          ...
          if (lineproplist[El->e[j]].BdryFormat == 1) {
            ds = sqrt(
              2. / (0.4 * PI * w * lineproplist[El->e[j]].Sig *
...
// #endif
        ...
        if (meshele[i].e[j] >= 0) {
          if (lineproplist[meshele[i].e[j]].BdryFormat == 0) {
            if (Coords == 0) {
              ...
              y = meshnode[meshele[i].p[j]].y;
              r = sqrt(x * x + y * y);
              if ((x == 0) && (y == 0)) {t = 0;} else {t = atan2(y, x) / DEG;}
              r /= units[LengthUnits];
              s = meshele[i].e[j];
    ...
    if (LinearFlag == false) {
      ...
      if (Iter > 5) {
        if ((res > lastres) && (Relax > 0.1)) {Relax /= 2.;} else {Relax += 0.1 * (1. - Relax);}
...
int FSolver::WriteHarmonic2D(CBigComplexLinProb & L)
{
  ...
  for (i = 0; i < NumNodes; i++) {
    fprintf(
      fp, "%.17g\t%.17g\t%.17g\t%.17g\t%i", meshnode[i].x / cf,
  ...
  for (i = 0; i < NumEls; i++) {
    ...
    if (!Aprev.empty()) {fprintf(fp, "\t%.17g\n", meshele[i].Jprev);} else {fprintf(fp, "\n");}
  }
