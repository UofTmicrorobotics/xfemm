...
LoadMeshErr FSolver::LoadMesh(bool deleteFiles)
{
...
#ifdef DEBUG
  {
    char buf[1048]; SNPRINTF(
      buf,
      sizeof(buf),
      "Found %i ages, line was: \"%s\"\n",
      NumAirGapElems,
      s);
    WarnMessage(buf);
  }
    ...
    for (k = 0; k <= age.totalArcElements; k++) {
        ...
        (qp.n3 < 0) )
      {
        std::string msg = std::string(
          "An error occured while reading file, quadNode has negative node number. ") +
  ...
  for (i = 0; i < k; i++) {
    ...
    if (elm.lbl < 0) {
      ...
      char buf[1028];
      SNPRINTF(buf, sizeof(buf), "The element number %i had label %i\n", i, elm.lbl);
      msg += std::string(buf);
      WarnMessage(msg.c_str());
      fclose(fp);
      if (deleteFiles) {
    ...
    if (!(elm.lbl < (int)labellist.size())) {
      ...
      SNPRINTF(
        buf,
        sizeof(buf),
        "The element number %i had label %i which is greater than the number of available labels (%i)\n",
        i + 1,
        elm.lbl + 1,
        (int)labellist.size());
      WarnMessage(buf);
...
bool FSolver::LoadAGEsFromSolution(FILE * fp)
{
  ...
  for (int i = 0; i < NumAirGapElems; i++) {
    ...
    for (int k = 0; k <= age.totalArcElements; k++) {
      ...
      {
        std::string msg = std::string(
          "An error occured while reading pbc file, quadNode has negative node number. ") +
...
void FSolver::GetFillFactor(int lbl)
{
  ...
  switch (wiretype) {
    ...
    case 4:     // 10% CCA
      c1 = 0.7270741505617485 + 0.8902950067721367 * fill + 0.11894736885885195 * fill * fill -
        0.12247276254503957 * fill * fill * fill;
      c2 = 0.006784920229549677 + 1.8942880489198526 * fill - 1.3631438759519217 * fill * fill +
        0.504431701685587 * fill * fill * fill;
      break;

    case 5:     // 15% CCA
      c1 = 0.7486913529860821 + 0.9042845510838825 * fill + 0.1361040321433224 * fill * fill -
        0.10652380745682069 * fill * fill * fill;
      c2 = 0.006790468527313965 + 1.8945509985370095 * fill - 1.3643501010185972 * fill * fill +
        0.5036765577982594 * fill * fill * fill;
      break;
  }
...
bool FSolver::runSolver(bool verbose)
{
  ...
  if (Frequency == 0) {
    ...
    if (ProblemType == PLANAR) {
      if (!previousSolutionFile.empty()) {
        WarnMessage(
          "Harmonic planar incremental permeability problems are work in progress. RESULTS WON'T BE VALID!\n");
      }
