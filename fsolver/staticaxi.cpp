...
int FSolver::StaticAxisymmetric(CBigLinProb & L)
{
  ...
  double l[3], p[3] = {0., 0., 0.}, q[3] = {0., 0., 0.}, g[3], be[3], u[3], v[3], res, lastres = 0.,
    dv, vol;
...
//            }
      ...
      switch (flag) {
        ...
        case 1:
          ...
          if (rn[0] < 1.e-06) {
            if (fabs(rn[1] - rn[2]) < 1.e-06) {R_hat = rn[2] / 2.;} else {
              R_hat = (rn[1] - rn[2]) / (2. * log(rn[1]) - 2. * log(rn[2]));
            }
          }
          if (rn[1] < 1.e-06) {
            if (fabs(rn[2] - rn[0]) < 1.e-06) {R_hat = rn[0] / 2.;} else {
              R_hat = (rn[2] - rn[0]) / (2. * log(rn[2]) - 2. * log(rn[0]));
            }
          }
          if (rn[2] < 1.e-06) {
            if (fabs(rn[0] - rn[1]) < 1.e-06) {R_hat = rn[1] / 2.;} else {
      ...
      for (j = 0; j < 3; j++) {
        for (k = j; k < 3; k++) {
          Mxy[j][k] += K * ((q[j] * rn[j]) * (g[j] / R)) * (p[k] * rn[k]) + K *
            ((q[k] * rn[k]) * (g[k] / R)) * (p[j] * rn[j]);
        }
      }
...
//                    str = fmatter.str();
      ...
      if (Iter == 0) {
        ...
        if (blockproplist[k].BHpoints != 0) {
          if (bIncremental == 0) {
            ...
            if (blockproplist[k].LamType > 0) {
              PrintMessage(
                "On-edge Lam Types not yet supported in incremental/frozen permeability problems");
              exit(0);
            }
    ...
    if (LinearFlag == false) {
      ...
      if (Iter > 5) {
        if ((res > lastres) && (Relax > 0.125)) {Relax /= 2.;} else {Relax += 0.1 * (1. - Relax);}
