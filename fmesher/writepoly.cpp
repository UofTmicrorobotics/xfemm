...
public:
   ...
   */
  bool initPointsWithMarkers(
    const nodelist_t & nodelst,
    const FemmProblem & problem,
    PointMarkerInfo info);
  /**
   ...
   * @return \c true on success, \c false on (allocation) error
   */
  bool initSegmentsWithMarkers(
    const linelist_t & linelst,
    const FemmProblem & problem,
    SegmentMarkerInfo info);
   ...
   * @return \c true on success, \c false on (allocation) error
   */
  bool initHolesAndRegions(
    const FemmProblem & problem,
    bool forceMaxMeshArea,
...
double fmesher::defaultMeshSizeHeuristics(
  const std::vector<std::unique_ptr<CNode>> & nodelst,
  bool doSmartMesh)
{
  if (nodelst.empty()) {
    return -1;
...
void fmesher::discretizeInputSegments(
  const FemmProblem & problem,
...
void fmesher::discretizeInputArcSegments(
  const FemmProblem & problem,
  std::vector<std::unique_ptr<CNode>> & nodelst,
  std::vector<std::unique_ptr<CSegment>> & linelst,
  SegmentFilter filter)
...
bool TriangulateHelper::writeTriangulationFiles(string PathName) const
{
  ...
  if (out.numberoftriangles > 0) {
    ...
    for (int i = 0, nexttriattrib = 0;
      i < (out.numberofcorners) * (out.numberoftriangles) - (out.numberofcorners - 1);
      i = i + (out.numberofcorners))
    {
...
int FMesher::DoPeriodicBCTriangulation(string PathName)
{
  ...
  for (i = 0; i < (int)problem->linelist.size(); i++) {
    if (ptlst[i]->t == 0) {
      ...
      // attempt at meshing
      problem->linelist[i]->MaxSideLength = problem->lengthOfLine(i) /
        ((double) problem->linelist[i]->cnt);
    }
  }
...
#ifdef DEBUG
  {
    ...
    SNPRINTF(
      buf,
      sizeof(buf),
      "writepoly: lineproplist.size() is %li\n",
  ...
  for (i = 0; i < (int)problem->lineproplist.size(); i++) {
#ifdef DEBUG
    {
      ...
      SNPRINTF(
        buf,
        sizeof(buf),
        "writepoly: searching for periodic boundaries, checking boundary %i called %s\n",
        i,
      ...
      (problem->lineproplist[i]->BdryFormat == 5))
    {
#ifdef DEBUG
      {
        ...
        SNPRINTF(
          buf,
          sizeof(buf),
          "writepoly: %s is periodic boundary, updating pbc\n",
          problem->lineproplist[i]->BdryName.c_str());
        WarnMessage(buf);
      }
#endif  // DEBUG
      ...
      pbc.antiPeriodic = problem->lineproplist[i]->isPeriodic(
        CBoundaryProp::PeriodicityType::AntiPeriodic);
  ...
  for (i = 0; i < (int)agelst.size(); i++) {
    if (agelst[i]->totalArcLength > 0) {         // if the AGE is actually in play
      ...
      altMaxSideLength = (360. / PI) * (agelst[i]->ro - agelst[i]->ri) /
        (agelst[i]->ro + agelst[i]->ri);
...
//    }
      ...
      if (pbclst[j]->BdryName == problem->linelist[i]->BoundaryMarkerName) {
        ...
        if (pbclst[j]->nseg == 2) {
          ...
          SNPRINTF(
            buf,
            sizeof(buf),
            "An (anti)periodic BC (named \"%s\") is assigned to more than two segments",
            pbclst[j]->BdryName.c_str());
          WarnMessage(buf);
          problem->undo();  problem->unselectAll();
          return -1;
        }
      ...
      if (pbclst[j]->BdryName == problem->arclist[i]->BoundaryMarkerName) {
        ...
        if (pbclst[j]->narc == 2) {
          ...
          SNPRINTF(
            buf,
            sizeof(buf),
            "An (anti)periodic BC (named \"%s\") is assigned to more than two arcs",
            pbclst[j]->BdryName.c_str());
          WarnMessage(buf);
          problem->undo();  problem->unselectAll();
          return -1;
  ...
  while (j < (int)pbclst.size()) {
    ...
    if ((pbclst[j]->nseg < 2) && (pbclst[j]->narc < 2)) {pbclst.erase(pbclst.begin() + j);} else {
      j++;
    }
  }
  ...
  for (j = 0; j < (int)pbclst.size(); j++) {
    ...
    if (pbclst[j]->nseg > 0) {
      ...
      if (fabs(
          problem->lengthOfLine(pbclst[j]->seg[0]) -
    ...
    if (pbclst[j]->narc > 0) {
      ...
      // same arc length
      if (fabs(
          problem->arclist[pbclst[j]->seg[0]]->ArcLength -
  ...
  for (n = 0; n < (int)agelst.size(); n++) {
    ...
    for (i = 0; i < (int)problem->arclist.size(); i++) {
      if ((problem->arclist[i]->IsSelected == false) &&
        (problem->arclist[i]->BoundaryMarkerName == agelst[n]->BdryName)) {
        problem->arclist[i]->IsSelected = true;
        a2.Set(problem->nodelist[problem->arclist[i]->n0]->x,
          problem->nodelist[problem->arclist[i]->n0]->y);
    ...
    for (k = 0; k < (int)myVector.size(); k++) {
      agelst[n]->nodeNums[k + 1] = myVector[k];
    }
  }
...
bool TriangulateHelper::initPointsWithMarkers(const TriangulateHelper::nodelist_t & nodelst,
  const FemmProblem & problem,
  PointMarkerInfo info)
{
...
bool TriangulateHelper::initSegmentsWithMarkers(const TriangulateHelper::linelist_t & linelst,
  const FemmProblem & problem,
  SegmentMarkerInfo info)
{
  // calling this method on an already initialized object would leak memory
...
bool TriangulateHelper::initHolesAndRegions(const FemmProblem & problem,
  bool forceMaxMeshArea,
  double defaultMeshSize)
{
  // calling this method on an already initialized object would leak memory
    ...
    for (const auto & label: problem.labellist) {
      ...
      if (label->isHole()) {
#ifdef DEBUG
        {
          char buf[1028];
          SNPRINTF(
            buf, sizeof(buf), "Adding hole (at (%g,%g)) to triangle input hole list\n",
            label->x, label->y);
          WarnMessage(buf);
        }
#endif  // DEBUG
    ...
    if (!label->isHole()) {
...
#ifdef DEBUG
      {
        ...
        SNPRINTF(
          buf,
          sizeof(buf),
          "Adding region (at (%g,%g)) with attribute value %g to triangle input region list\n",
          label->x,
          label->y,
          in.regionlist[j + 2]);
        WarnMessage(buf);
...
#ifdef XFEMM_BUILTIN_TRIANGLE
  int tristatus = ::triangulate(cmdline,
      &in,
      &out,
...
bool TriangulateHelper::writePolyFile(string filename, std::string comment) const
{
  ...
  for (int i = 0; i < in.numberofpoints; i++) {
    polyFile << i << "\t" << in.pointlist[2 * i] << "\t" << in.pointlist[2 * i + 1] << "\t" <<
      in.pointmarkerlist[i] << "\n";
  }

  polyFile << in.numberofsegments << "\t1\n";
  for (int i = 0; i < in.numberofsegments; i++) {
    polyFile << i << "\t" << in.segmentlist[2 * i] << "\t" << in.segmentlist[2 * i + 1] << "\t" <<
