...
template<class PointPropT,
  ...
  class MeshElementT
>
FEASolver<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT,
  MeshElementT>::FEASolver()
: FileFormat(-1)
  , Precision(1.e-08)
  , MinAngle(0.)
  , Depth(-1)
...
template<class PointPropT,
  ...
  class MeshElementT
>
void FEASolver<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT,
  MeshElementT>::CleanUp()
{
  FileFormat = -1;
  Precision = 1.e-08;
  MinAngle = 0.;
...
template<class PointPropT,
  ...
  class MeshElementT
>
bool FEASolver<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT,
  MeshElementT>::LoadProblemFile(std::string & file)
{
  std::ifstream input;
  ...
  while (input.good() && success) {
    ...
    if (token == "[lengthunits]") {
      ...
      if (token == "inches") {LengthUnits = LengthInches;} else if (token == "millimeters") {
        LengthUnits = LengthMillimeters;
      } else if (token == "centimeters") {
        LengthUnits = LengthCentimeters;
      } else if (token == "mils") {LengthUnits = LengthMils;} else if (token == "microns") {
        LengthUnits = LengthMicrometers;
...
#ifdef DEBUG_PARSER
      {
        char buf[1048]; SNPRINTF(buf,
          sizeof(buf),
          "feasolver reserved space for %i block labels, labellist length is %i\n",
          NumBlockLabels,
          (int) labellist.size());
        WarnMessage(buf);
      }
...
//            }
#ifdef DEBUG
      {
        char buf[1048]; SNPRINTF(buf,
          sizeof(buf),
          "feasolver: Expected %i block labels, labellist length is %i\n",
          NumBlockLabels,
          (int) labellist.size());
        WarnMessage(buf);
      }
...
>
bool FEASolver<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT,
  MeshElementT>::handleToken(const string &, istream &, ostream &)
{
  // token not handled
  return false;
}
...
template<class PointPropT,
  ...
  class MeshElementT
>
std::string FEASolver<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT,
  MeshElementT>::getErrorString(LoadMeshErr err)
{
  switch (err) {
    case (NOERROR):
      return std::string();
