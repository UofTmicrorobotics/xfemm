...
bool femm::FemmProblem::addArcSegment(femm::CArcSegment & asegm, double tol)
{
  ...
  double t;
  if (tol == 0) {
    if (nodelist.size() < 2) {t = 1.e-08;} else {
      CComplex p0, p1;
      p0 = nodelist[0]->CC();
      p1 = p0;
    ...
    case 0:  // One arc and one line
      {
        ...
        if (m > 1) {
          if (abs(v[0] - p0) < abs(v[1] - p0)) {j = 0;} else {j = 1;}
    ...
    case 2:  // Two lines
      {
        ...
        if (linelist[seg[0]]->n0 == n) {p1 = nodelist[linelist[seg[0]]->n1]->CC();} else {
          p1 = nodelist[linelist[seg[0]]->n0]->CC();
        }

        if (linelist[seg[1]]->n0 == n) {p2 = nodelist[linelist[seg[1]]->n1]->CC();} else {
    ...
    case -2:  // Two arcs
      {
        ...
        // We then pick the solution that has the center point closest to the point to be removed.
        if (j > 1) {
          if (abs(p[0] - c0) < abs(p[1] - c0)) {j = 0;} else {j = 1;}
        } else {j = 0;       // The index of the winning case is in j....
...
int femm::FemmProblem::getArcArcIntersection(const femm::CArcSegment & arc0,
  const femm::CArcSegment & arc1,
  CComplex * p) const
{
...
int femm::FemmProblem::getLineArcIntersection(const femm::CSegment & seg,
  const femm::CArcSegment & arc,
  CComplex * p) const
{
...
void femm::FemmProblem::mirrorCopy(double x0,
  double y0,
  double x1,
  double y1,
  femm::EditMode selector)
{
...
void femm::FemmProblem::translateCopy(double incx, double incy, int ncopies,
  femm::EditMode selector)
{
  assert(selector != EditMode::Invalid);
  for (int nc = 0; nc < ncopies; nc++) {
