...
static FILE * getfilebyref(lua_State * L, IOCtrl * ctrl, int inout)
{
  ...
  if (f == NULL) {
    luaL_verror(
      L, "global variable `%.10s' is not a file handle",
      filenames[inout]);
  }
  return f;
}
...
static void setfilebyname(
  lua_State * L, IOCtrl * ctrl, FILE * f,
  const char * name)
{
...
static int io_read(lua_State * L)
{
  ...
  int n;
  if (f) {firstarg++;} else {f = getfilebyref(L, ctrl, INFILE); /* get _INPUT */
  }
  lua_pop(L, 1);
...
static int io_write(lua_State * L)
{
  ...
  int status = 1;
  FILE * f = gethandle(L, ctrl, arg);
  if (f) {arg++;} else {f = getfilebyref(L, ctrl, OUTFILE); /* get _OUTPUT */
  }
  for (; arg <= lastarg; arg++) {
