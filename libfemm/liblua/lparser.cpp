...
static void check_match(LexState * ls, int what, int who, int where)
{
  if (ls->t.token != what) {
    if (where == ls->linenumber) {
      ...
      sprintf(
        buff, "`%.20s' expected (to close `%.20s' at line %d)",
        t_what, t_who, where);
      luaK_error(ls, buff);
    }
  }
...
static void pushclosure(LexState * ls, FuncState * func)
{
  ...
  luaM_growvector(
    ls->L, f->kproto, f->nkproto, 1, Proto *,
...
static int listfields(LexState * ls)
{
  ...
  while (ls->t.token == ',') {
    ...
    n++;
    luaX_checklimit(
      ls, n / LFIELDS_PER_FLUSH, MAXARG_A,
...
static void chunk(LexState * ls)
{
  ...
  while (!islast && !block_follow(ls->t.token)) {
    ...
    optional(ls, ';');
    LUA_ASSERT(
      ls->fs->stacklevel == ls->fs->nactloc,
