...
CComplex CComplex::Sqrt()
{
  ...
  if ((re == 0) && (im == 0)) {w = 0;} else if (fabs(re) > fabs(im)) {
    z = im / re;
...
{
  if (im == 0) {sprintf(s, "%.16g", re);} else if (im == 1) {
    if (re == 0) {sprintf(s, "I");} else {sprintf(s, "%.16g+I", re);}
  } else if (im == -1) {
    if (re == 0) {sprintf(s, "-I");} else {sprintf(s, "%.16g-I", re);}
  } else if (im < 0) {
    if (re != 0) {sprintf(s, "%.16g-I*%.16g", re, fabs(im));} else {
      sprintf(s, "-I*%.16g", fabs(im));
    }
  } else if (im > 0) {
...
{
  if (im == 0) {sprintf(s, "%g", re);} else if (im == 1) {
    if (re == 0) {sprintf(s, "I");} else {sprintf(s, "%g+I", re);}
  } else if (im == -1) {
    if (re == 0) {sprintf(s, "-I");} else {sprintf(s, "%g-I", re);}
  } else if (im < 0) {
    if (re != 0) {sprintf(s, "%g-I*%g", re, fabs(im));} else {sprintf(s, "-I*%g", fabs(im));}
  } else if (im > 0) {
    if (re != 0) {sprintf(s, "%g+I*%g", re, im);} else {sprintf(s, "I*%g", im);}
  } else {s[0] = '\0';}
...
CComplex sqrt(const CComplex & x)
{
  ...
  if ((x.re == 0) && (x.im == 0)) {w = 0;} else if (fabs(x.re) > fabs(x.im)) {
    z = x.im / x.re;
...
CComplex atan2(const CComplex & y, const CComplex & x)
{
  ...
  return arg((x + I * y) / sqrt(x * x + y * y)) - I *
         log(abs(x + I * y) / sqrt(abs(x * x + y * y)));
}
