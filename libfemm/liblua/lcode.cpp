...
static void luaK_patchlistaux(
  FuncState * fs, int list, int target,
...
static void codelineinfo(FuncState * fs)
{
  ...
  if (ls->lastline > fs->lastline) {
    luaM_growvector(
      fs->L, f->lineinfo, f->nlineinfo, 2, int,
...
int luaK_code2(FuncState * fs, OpCode o, int arg1, int arg2)
{
  ...
  luaM_growvector(
    fs->L, fs->f->code, fs->pc, 1, Instruction,
