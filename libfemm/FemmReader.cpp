...
template<class PointPropT,
  ...
  class BlockLabelT
>
FemmReader<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT>::FemmReader(
...
template<class PointPropT,
  ...
  class BlockLabelT
>
ParserResult FemmReader<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT>::parse(
  const std::string & file)
{
  ...
  while (input.good() && success) {
    ...
    if (token == "[lengthunits]") {
      ...
      if (token == "inches") {
        problem->LengthUnits = LengthInches;
      } else if (token == "millimeters") {
        problem->LengthUnits = LengthMillimeters;
      } else if (token == "centimeters") {
        problem->LengthUnits = LengthCentimeters;
      } else if (token == "mils") {
        problem->LengthUnits = LengthMils;
      } else if (token == "microns") {
        problem->LengthUnits = LengthMicrometers;
      } else if (token == "meters") {problem->LengthUnits = LengthMeters;} else {
        err << "Unknown length unit: " << token << "\n";
      }
      continue;
    }
    ...
    if (token == "[coordinates]") {
      ...
      if (token == "cartesian") {problem->Coords = CART;} else if (token == "polar") {
        problem->Coords = POLAR;
    ...
    if (token == "[problemtype]") {
      ...
      if (token == "planar") {problem->problemType = PLANAR;} else if (token == "axisymmetric") {
...
template<class PointPropT,
  ...
  class BlockLabelT
>
bool FemmReader<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT,
  BlockLabelT>::ignoreUnhandledTokens() const
{
...
template<class PointPropT,
  ...
  class BlockLabelT
>
void FemmReader<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT,
  BlockLabelT>::setIgnoreUnhandledTokens(bool value)
{
...
template<class PointPropT,
  ...
  class BlockLabelT
>
bool FemmReader<PointPropT, BoundaryPropT, BlockPropT, CircuitPropT, BlockLabelT>::handleToken(
  const string &,
  istream &,
  ostream &)
{
...
MagneticsReader::MagneticsReader(std::shared_ptr<FemmProblem> problem,
  SolutionReader * r,
  ostream & errorpipe)
...
HeatFlowReader::HeatFlowReader(std::shared_ptr<FemmProblem> problem,
  SolutionReader * r,
  ostream & errorpipe)
: FemmReader_type(problem, r, errorpipe)
{
...
ElectrostaticsReader::ElectrostaticsReader(std::shared_ptr<FemmProblem> problem,
  ostream & errorpipe)
: FemmReader_type(problem, errorpipe)
{
}
