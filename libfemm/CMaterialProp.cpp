...
void CMMaterialProp::toStream(ostream &) const
{
  assert(
    false &&
    "CMMaterialProp::toStream() should never be called. Did you mean to call CMSolverMaterialProp::toStream()?");
}

void CMMaterialProp::GetMu(
  const CComplex b1, const CComplex b2,
  CComplex & mu1, CComplex & mu2)
{
  ...
  // Rats! The material is nonlinear.
    ...
    if (LamType == 1) {
      biron = sqrt((b1 / LamFill) * (b1 / LamFill) + b2 * b2);
      if (abs(biron) < 1.e-08) {muiron = 1. / slope[0];} else {muiron = biron / GetH(biron);}
      mu1 = muiron * LamFill;
...
void CMMaterialProp::GetMu(const double b1, const double b2, double & mu1, double & mu2)
{
  ...
  // Rats! The material is nonlinear.
    ...
    if (LamType == 1) {
      biron = sqrt((b1 / LamFill) * (b1 / LamFill) + b2 * b2);
      if (biron < 1.e-08) {muiron = 1. / Re(slope[0]);} else {muiron = biron / GetH(biron);}
      mu1 = muiron * LamFill;
      mu2 = 1. / (LamFill / muiron + (1. - LamFill) / muo);
    }
...
void CMMaterialProp::incrementalPermeability(
  const double B,
  const double w,
  CComplex & mu1,
  CComplex & mu2)
...
CMSolverMaterialProp CMSolverMaterialProp::fromStream(
  std::istream & input,
  std::ostream & err,
  PropertyParseMode mode)
{
  using namespace femm;
...
CHMaterialProp CHMaterialProp::fromStream(
  std::istream & input,
  std::ostream & err,
  PropertyParseMode mode)
{
