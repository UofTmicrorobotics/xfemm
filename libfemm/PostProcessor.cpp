...
void PostProcessor::addContourPointFromNode(double mx, double my)
{
  if (!problem->nodelist.empty()) {
    ...
    if (abs(x - y) < 1.e-08) {
      for (int k = 0; k < (int)problem->arclist.size(); k++) {
        if ((problem->arclist[k]->n0 == n1) && (problem->arclist[k]->n1 == n0)) {
          double d2 = problem->shortestDistanceFromArc(
            CComplex(mx, my),
    ...
    if (arcno >= 0) {
      ...
      int arcsegments = (int) ceil(
        problem->arclist[k]->ArcLength / problem->arclist[k]->MaxSideLength);
      if (reverseOrder) {
...
bool PostProcessor::isSameMaterial(
  const femmsolver::CElement & e1,
  const femmsolver::CElement & e2) const
{
...
bool PostProcessor::makeMask()
{
  ...
  for (int i = 0; i < NumNodes; i++) {
    ...
    if (meshnodes[i]->Q != -2) {L.V[i] = 0;} else {L.V[i] = -1;}
  }
  ...
  if (problem->filetype == femm::FileType::ElectrostaticsFile) {
    for (int i = 0; i < NumNodes; i++) {
      const femmsolver::CSMeshNode * node =
        reinterpret_cast<femmsolver::CSMeshNode *>(meshnodes[i].get());
  ...
  for (int i = 0; i < NumNodes; i++) {
    if (L.V[i] > 0.5) {meshnodes[i]->msk = 1;} else {meshnodes[i]->msk = 0;}}
...
void PostProcessor::getNodalD(CComplex * d, int N) const
{
  ...
  if (problem->filetype != FileType::HeatFlowFile &&
    problem->filetype != FileType::ElectrostaticsFile)
  {
    return;
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (flag == false) {
      ...
      det = (-(ii * xy * xy) + 2 * xi * xy * yi - xx * yi * yi - xi * xi * yy + ii * xx * yy) *
        LengthConv[problem->LengthUnits];

      if (det == 0) {d[i] = elem->D;} else {
        Ex =
          (iv * xy * yi - xv * yi * yi - ii * xy * yv + xi * yi * yv - iv * xi * yy + ii * xv *
          yy) / det;
        Ey =
          (iv * xi * xy - ii * xv * xy + xi * xv * yi - iv * xx * yi - xi * xi * yv + ii * xx *
          yv) / det;
        switch (problem->filetype) {
          case FileType::ElectrostaticsFile:
            {
              const auto bprop =
                reinterpret_cast<CSMaterialProp *>(problem->blockproplist[elem->blk].get());
          ...
          case FileType::HeatFlowFile:
            {
              const auto nodej = reinterpret_cast<femmsolver::CHMeshNode *>(meshnodes[j].get());
              const auto bprop =
                reinterpret_cast<CHMaterialProp *>(problem->blockproplist[elem->blk].get());
...
void PostProcessor::getPointD(
  double x, double y, CComplex & D,
  const femmsolver::CElement & element) const
{
  // this method is only valid for heatflow and electrostatics problems; otherwise punt
  if (problem->filetype != FileType::HeatFlowFile &&
    problem->filetype != FileType::ElectrostaticsFile)
